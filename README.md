...
> 子系统的功能调用多且复杂的时候，可以定义一个更高层的外观接口，聚合子系统，对客户的提供统一简洁的调用方式。

角色：
1. 子系统：功能的执行者
2. 外观：调度子系统完成任务，对客户端提供使用接口
3. 客户端：调用外观接口

## 十二、享元模式

> 减少创建对象的数量，尝试重用现有对象，如果没有则新建。

何时使用：
1. 系统中有大量对象，这些对象消耗大量内存
2. 对象的状态大部分可以外部化（比如，棋子的坐标）
3. 对象可以按照内蕴状态分为很多组，当把外蕴对象从对象中剔除出来时，每一组对象都可以用一个对象来代替（比如棋子的颜色，剔除外部坐标，同颜色的棋子没区别）
4. 系统不依赖于这些对象的身份，对象是不可分辨的

UML类图：
![](https://uploader.shimo.im/f/7hYZl7ZH3VMiG4lG.jpg)
...
> 为目标对象添加代理，通过代理对象访问目标对象，代理可以在目标对象的方法前后加入一些其他功能（安全验证，日志输出），不会影响目标的代码。

实现方式：
1. 静态代理
    1. 缺点：
        - 代理和目标都实现了同一个接口，接口改变，两者都需要变化
        - 要为每一个目标类都创建一个代理类
2. JDK动态代理
    1. 实现：
        - 代理工厂聚合目标对象
...
3. cglib动态代理
    1. 实现：
        - 代理工厂聚合目标对象
        - 通过enhancer创建代理对象，设置目标类型，回调函数
        - 代理工厂实现MethodInterceptor接口的intercept方法，处理目标方法的调用
    2. 优点：
...
> 将军指挥作战无需指挥每个士兵，只需要下命令调度士兵即可。
对编程而言，请求者和接收者解耦，请求者聚合命令，命令聚合接受者执行任务。

角色：
1. 命令接口：execute
2. 具体的命令：聚合接收者对象，实现命令接口
3. 接收者：执行具体的任务
4. 请求者：聚合命令对象

UML类图：

![](https://uploader.shimo.im/f/ahl5Q7O6k4g0SJLV.jpg)
